...
def first_day_of_month(time_tuple: struct_time):
    return date(time_tuple.tm_year, time_tuple.tm_mon, 1)

def last_day_of_month(time_tuple: struct_time):
...
def print_title(time_tuple: struct_time):
    print('     {}'.format(strftime('%B %Y', first_day_of_month(time_tuple).timetuple())))
    wday_names = ['MO', 'Tu', 'We', 'Th', 'Fr', 'Sa', 'Su']
    for wday_name in wday_names:
        print('{} '.format(wday_name), end = '')
    print()

def print_days(time_tuple: struct_time):
